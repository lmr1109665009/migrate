...
public class RequestUtil {
	...
	public static Byte getByte(HttpServletRequest request, String key) {
		return getByte(request, key, (byte)0);
	}

	/**
	 * 从Request中取得Byte值 对字符串,如无值则返回缺省值
	 * 
	 * @param request
	 * @param key
	 * @param defaultValue
	...
	public static Date getDate(HttpServletRequest request, String key,
		...
		return DateUtil.getDate(str, style);
	}
		...
		while (params.hasMoreElements()) {
			...
			if (values.length > 0 && StringUtils.isNotEmpty(values[0])) {
				if (values.length == 1) {
					...
					String str=ArrayUtil.contact(values, ",");
					map.put(key, str);
				}
			}
		}
	...
	private static Object getObjValue(String type, String paramValue) {
		...
		// date begin
			...
			value = DateUtil.getDate(paramValue, "yyyy-MM-dd");
		}
		// date end
		else if ("DG".equals(type)) {
			value = DateUtil.getNextDays(
					DateUtil.getDate(paramValue, "yyyy-MM-dd"), 1);
